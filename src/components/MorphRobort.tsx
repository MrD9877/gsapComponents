import gsap, { Bounce, Elastic, SteppedEase } from "gsap";
import DrawSVGPlugin from "gsap/DrawSVGPlugin";
import MorphSVGPlugin from "gsap/MorphSVGPlugin";
import React, { useEffect, useRef } from "react";
import * as THREE from "three";

export default function MorphRobort() {
  gsap.registerPlugin(MorphSVGPlugin, DrawSVGPlugin);
  const sceneRef = useRef<THREE.Scene>(null);
  const cameraRef = useRef<THREE.PerspectiveCamera>(null);
  const rendererRef = useRef<THREE.WebGLRenderer>(null);
  const webglRef = useRef<HTMLDivElement>(null);
  const startGeometry = useRef<THREE.BufferGeometry<THREE.NormalBufferAttributes>>(null);
  const stars = useRef<THREE.Points<THREE.BufferGeometry<THREE.NormalBufferAttributes>, THREE.PointsMaterial, THREE.Object3DEventMap>>(null);
  const svg = useRef<SVGSVGElement>(null);
  const note1Ref = useRef<SVGPathElement>(null);
  const note2Ref = useRef<SVGPathElement>(null);
  const robotRef = useRef<SVGGElement>(null);
  const isLeft = useRef<boolean>(false);
  const count = 5000;
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const robortTlRef = useRef<gsap.core.Timeline>(null);
  ...
  function animation() {
    ...
    robortTlRef.current = gsap.timeline({
      repeat: -1,
    });
    const robotTl = robortTlRef.current;
    robotTl.to(
      "#robot",
      {
        x: 100,
    ...
    //top right circle
    const circlesTop = gsap.utils.toArray(".circles-top") as SVGPathElement[];
    circlesTop.forEach((top) => {
      gsap.to(top, {
        rotation: 360,
        duration: 10,
        transformOrigin: "center",
    ...
    for (let i = 0; i < 3; i++) {
      const clone1 = note1Ref.current?.cloneNode(true) as HTMLElement;
      const clone2 = note2Ref.current?.cloneNode(true) as HTMLElement;
      ...
      svg.current?.appendChild(clone1);
      svg.current?.appendChild(clone2);
    }
  ...
  function init() {
    ...
    startGeometry.current = new THREE.BufferGeometry();

    const positions = new Float32Array(count * 3);
    for (let i = 0; i < count; i++) {
      positions[i] = Math.random() * 600 - 300;
    }
    startGeometry.current.setAttribute("position", new THREE.BufferAttribute(positions, 3));

    const sprite = new THREE.TextureLoader().load("https://i.postimg.cc/T25jx3S9/circle-05.png");
    ...
    stars.current = new THREE.Points(startGeometry.current, starMaterial);
    sceneRef.current.add(stars.current);
    ...
    rendererRef.current.setClearColor("#282e39", 0.5);
    ...
    function animate() {
      if (!startGeometry.current || !stars.current || !rendererRef.current || !sceneRef.current || !cameraRef.current) return;
      for (let i = 0; i < count; i++) {
        const i3 = i * 3;

        startGeometry.current.attributes.position.array[i3 + 1] -= 3;

        if (startGeometry.current.attributes.position.array[i3 + 1] < -100) {
          startGeometry.current.attributes.position.array[i3 + 1] = 300;
        }
      }
      startGeometry.current.attributes.position.needsUpdate = true;
  ...
  return (
    <div style={{ width: `${width}px`, height: `${height}px` }} className="bg-[#34496a]  flex overflow-hidden justify-center items-center">
      <div ref={webglRef} id="webgl" style={{ width: `${width}px`, height: `${height}px` }} className="absolute"></div>
      <div id="bg-box" style={{ width: `${width + 2}px`, height: `${height + 2}px` }} className=" absolute ">
        <svg ref={svg} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600">
      ...
      <div style={{ width: `${width}px`, height: `${height}px` }} className="svg-box  absolute ">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 600">
          ...
          <g id="Ship">
            ...
            <g ref={robotRef} onClick={sayHello} id="robot">
                ...
                <g clipPath="url(#clipPath-2)">
                  <g id="faces">
                    ...
                    <g id="face-2" data-name="face">
                      ...
                      <g className="eyes" id="eyes-2" fill="#0ff" data-name="eyes">
            ...
            <path ref={note1Ref} id="note-1" fill="none" d="M180 317l-3.5-3.8a1 1 0 00-1.7.7v8.1a6 6 0 00-2-.3c-2.5 0-4.6 1.6-4.6 3.5s2 3.5 4.7 3.5 4.6-1.5 4.6-3.5a3 3 0 00-.7-1.9v-6.8l1.7 1.8a1 1 0 101.5-1.4z" />
            <path ref={note2Ref} id="note-2" fill="none" d="M203.4 323.4v-9.5a1 1 0 00-1-1h-9.3a1 1 0 00-1 1v8.1a6 6 0 00-2-.3c-2.5 0-4.6 1.6-4.6 3.5s2 3.5 4.7 3.5 4.6-1.5 4.6-3.5a2.9 2.9 0 00-.7-1.9V315h7.3v7.1a5.8 5.8 0 00-1.9-.3c-2.6 0-4.7 1.6-4.7 3.5s2.1 3.5 4.7 3.5 4.7-1.5 4.7-3.5a2.9 2.9 0 00-.8-1.8z" />
          </g>
        </svg>
      </div>
      <canvas width={width} height={height} style={{ width: `${width}px`, height: `${height}px` }} ref={canvasRef} className="webgl2 absolute top-0 left-0 z-10 outline-none"></canvas>
    </div>
  );
}
